// 1:1 wzor z apps/formularze-ecodan/src/debug.js - patrz tam po uzasadnienie.
...
export function getDebugRoot(page = null, fallbackOutputRoot = null) {
  const root = fallbackOutputRoot || page?.__varmeroOutputRoot || getOutputRoot();
...
export async function saveDebug(page, prefix, details = '', extra = null, outputRoot = null) {
  ...
  try {
    if (page && !page.isClosed()) await fs.writeFile(html, await page.content(), 'utf8');
  } catch {}
  ...
  try {
    if (extra !== null) await fs.writeFile(json, JSON.stringify(extra, null, 2), 'utf8');
  } catch {}
...
export async function cleanupDebugArtifact(debug) {
  ...
  const paths = [debug.screenshot, debug.html, debug.log, debug.json].filter(Boolean);
