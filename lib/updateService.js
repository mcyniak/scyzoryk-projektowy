...
// Audyt 2026-08-17 (zlapane live na produkcji, dwie proby aktualizacji pod
// rzad, obie exitCode 5 - "Scyzoryk.exe wciaz w uzyciu"): StopResidentTrayProcesses
// po stronie C# (skan po nazwie+sciezce, ten sam mechanizm co StopOwnedProcesses)
// zglosil ZERO trafien mimo ze rezydentna ikona w zasobniku realnie zyla i
// naprawde blokowala plik (potwierdzone niezaleznie przez RestartManager w
// logu Inno Setup) - dokladnie ta sama niedeterministyczna zawodnosc
// Process.GetProcessesByName, ktora wczesniej (audyt 2026-08-12) pomijala
// glownego nadzorce server.js. Dla nadzorcy server.js PID jest znany z
// calkowita pewnoscia (wlasny process.pid) - dla rezydentnej ikony NIE MA
// takiego pewnego zrodla po stronie Node (to OSOBNY, niepowiazany proces C#),
// wiec NotifyIconTrayHost.cs sam zapisuje tu wlasny PID w momencie przejecia
// ikony (patrz InstallPaths.ResidentTrayPidFilePath) - odczytujemy go tutaj
// TUZ PRZED spawnowaniem aktualizatora. Plik moze byc nieaktualny/nieobecny
// (brak rezydentnej ikony, stary plik po awarii) - to NIE jest blad, po
// prostu nie mamy dodatkowej podpowiedzi (UpdateApplier.cs i tak weryfikuje
// nazwe+sciezke przed zabiciem, nigdy nie ufa temu PID bezwarunkowo).
function readResidentTrayPid() {
  const pidPath = path.join(getDataRoot(), 'runtime', 'resident-tray.pid');
  let raw;
  try {
...
// Audyt/incydent na zywo (2026-08-06): --apply-update uruchamia KOPIE
// Scyzoryk.exe z katalogu aktualizacji (launcherExePath, patrz nizej), nie z
// prawdziwego katalogu instalacji - InstallPaths.FromBaseDirectory() po
// stronie C# dla TEJ kopii wskazywalby na katalog aktualizacji, wiec
// StopOwnedProcesses szukal node.exe pod zla sciezka (nie znajdowal
// prawdziwego procesu do zatrzymania), a restart probowal odpalic
// nieistniejacy plik w tym samym zlym katalogu. Prawdziwy katalog instalacji
// (rootDir) jest teraz jawnie przekazywany jako 4. argument, zamiast pozwalac
// launcherowi zgadywac go na podstawie wlasnego, celowo przeniesionego
// polozenia.
function buildUpdaterInvocation({ launcherExePath, installerPath, updateRoot, expectedVersion, installDir }) {
  const args = ['--apply-update', installerPath, expectedVersion, installDir, String(process.pid)];
  // 6. argument (PID rezydentnej ikony w zasobniku, jesli jakas aktywna) - patrz
  // readResidentTrayPid powyzej dla pelnego uzasadnienia (audyt 2026-08-17).
  // Dopisywany TYLKO gdy istnieje, zeby ArgsParser.cs (Length >= 6) nadal
  // poprawnie odrozniał "brak" od pustego stringa.
  const residentTrayPid = readResidentTrayPid();
  if (residentTrayPid) args.push(residentTrayPid);
