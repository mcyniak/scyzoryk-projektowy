...
function buildUpdaterInvocation({ launcherExePath, installerPath, updateRoot, expectedVersion, installDir }) {
  return {
    exe: launcherExePath,
    // 5. argument (wlasny PID) - audyt 2026-08-12 (zlapane live na produkcji):
    // StopOwnedProcesses po stronie C# dopasowuje procesy po nazwie+sciezce, co przy
    // realnej aktualizacji jednorazowo pominelo WLASNIE TEN proces (nadzorce server.js,
    // ktory wlasnie wywoluje ta funkcje) - instalator probowal nadpisac pliki pod
    // dzialajacym procesem. Przekazujemy wlasny process.pid jawnie, zeby UpdateApplier.cs
    // mial gwarantowany, gotowy do uzycia PID do dobicia niezaleznie od tego, czy szerszy
    // skan po nazwie/sciezce go zobaczy (patrz EnsureParentProcessStopped w UpdateApplier.cs).
    args: ['--apply-update', installerPath, expectedVersion, installDir, String(process.pid)],
