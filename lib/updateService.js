...
function createUpdateService(options) {
  ...
  async function runInstallFlow(release) {
    ...
    if (updaterChild && typeof updaterChild.on === 'function') {
      ...
      // Proces aktualizatora moze tez wystartowac poprawnie (bez zdarzenia
      // 'error'), a mimo to od razu sie wywalic - np. run-update.ps1 zostal
      // uszkodzony przy kopiowaniu, albo zasady wykonywania skryptow na danym
      // komputerze mimo wszystko go zablokowaly. run-update.ps1 z zalozenia
      // czeka 2s zanim zabije ten (nasz) proces Node, wiec wczesniejszy,
      // nieoczekiwany 'exit' w tym oknie zdazymy jeszcze zauwazyc i pokazac -
      // bez tego taka awaria byla rownie cicha jak brakujacy listener 'error'.
      updaterChild.on('exit', (code, signal) => {
        if (code === 0 || code === null) return;
        const message = `Proces aktualizatora zakonczyl sie natychmiast z kodem ${code}${signal ? ` (sygnal ${signal})` : ''} - aktualizacja nie zostala wykonana.`;
        log('error', 'update-process-exited-early', { message });
        setState({ state: 'error', error: message });
        persistState();
      });
    }
