...
async function downloadToPartialFile(urlString, destPath, options = {}) {
  ...
  // Czeka az strumien zapisu do pliku FAKTYCZNIE zamknie uchwyt pliku (zdarzenie
  // 'close'), zanim sprobujemy go usunac. Bez tego, na Windows proba usuniecia
  // pliku, ktory WriteStream wciaz trzyma otwarty (np. po przerwanym polaczeniu),
  // konczyla sie EPERM/"operation not permitted" - zlapane realnie w CI: test
  // symulujacy przerwane pobieranie failowal na Windows, bo out.destroy() nie
  // byl wywolywany dla wszystkich sciezek bledu i/lub nic nie czekalo na
  // faktyczne zwolnienie uchwytu przed fs.unlinkSync.
  function destroyOut() {
  ...
  try {
    await new Promise((resolve, reject) => {
      ...
      res.on('error', fail);
      out.on('error', fail);
      // Czekamy na 'close' (uchwyt pliku FAKTYCZNIE zwolniony), nie tylko na
      // wczesniejsze 'finish' (dane zapisane, ale fd moze byc jeszcze otwarty)
      // - wywolujacy (updateService.js) od razu robi fs.renameSync na tym
      // pliku, co na Windows wymaga zeby nikt go juz nie trzymal otwartego.
