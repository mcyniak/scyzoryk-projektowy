...
function setupProcessDiagnostics(appName, rootDir) {
  ...
  process.on('unhandledRejection', err => {
    ...
    log('fatal', 'unhandledRejection', { message: err?.message || String(err), stack: err?.stack || '', report });
    setTimeout(() => process.exit(1), 50).unref();
  });
...
// sztywno jednego - potrzebne np. do rownoleglych zapytan sieciowych z
// ograniczeniem (zeby nie zaczely wracac bledy limitu zapytan).
