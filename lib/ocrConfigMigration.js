...
function saveUserOcrConfig({ keyFileContent, location, processorId, projectId }) {
  ...
  // Audyt: zlapane realnie na produkcji - uzytkownik wpisal "UE" (naturalny
  // polski skrot "Unia Europejska") zamiast "eu". documentAiEngine.js
  // buduje z tej wartosci host API WPROST (`${location}-documentai.googleapis.com`),
  // wielkosc liter ma znaczenie - "UE-documentai.googleapis.com" po prostu
  // nie istnieje, co konczylo sie mylacym "12 UNIMPLEMENTED: HTTP 404" bez
  // zadnej wskazowki, ze to literowka regionu. Regiony Document AI sa zawsze
  // pisane malymi literami, wiec normalizacja jest zawsze bezpieczna (nigdy
  // nie psuje juz-poprawnej wartosci).
  const resolvedLocation = String(location || '').trim().toLowerCase();
