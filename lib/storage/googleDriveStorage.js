...
function createStorageClient(rootPath) {
  ...
  async function browseViaRclone(relativePath = '.') {
    ...
    let stdout;
    try {
      // 45s (nie 20s) - zmierzone bezposrednio: listowanie najwyzszego
      // poziomu Dysku (najwiecej pozycji, kazda sprawdzana pod katem
      // mozliwosci eksportu) zajmuje realnie ok. 34s przez siec.
      ({ stdout } = await execFileAsync('rclone', ['lsjson', remoteArg, '--drive-export-formats', 'xlsx'], { timeout: 45000, maxBuffer: 10 * 1024 * 1024 }));
    } catch (err) {
