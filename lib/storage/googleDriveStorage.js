...
// Martwy/zawieszony punkt montowania FUSE (rclone dziala, ale utknal np. na
// wolnym/martwym polaczeniu do API Google) potrafi sprawic, ze zwykle
// "readdir" na zamontowanej sciezce wisi bez konca - bez timeoutu jeden taki
// przypadek zawieszalby dane zadanie HTTP na zawsze (nigdy nie dostaje
// odpowiedzi), zamiast pokazac uzytkownikowi czytelny blad. Timeout NIE
// przerywa samego wywolania readdir (Node/FUSE nie daje na to prostego
// sposobu) - po prostu przestaje na nie czekac i zwraca blad wolajacemu.
...
function createStorageClient(rootPath) {
  ...
  // Prawdziwy korzen montowania rclone - MOZE byc INNY niz ROOT tego klienta,
  // jesli ROOT jest zawezonym podkatalogiem (np. Karty katalogowe dostaja
  // SCYZORYK_PROJECTS_ROOT, ktory per .env.example bywa podkatalogiem calego
  // montowania, nie samym jego korzeniem). toRcloneArg ponizej MUSI liczyc
  // sciezke wzgledem PRAWDZIWEGO korzenia montowania - liczac ja wzgledem
  // zawezonego ROOT, traciloby prefiks podkatalogu i trafialoby w zupelnie
  // inne miejsce na Dysku. Fallback na ROOT zachowuje dzisiejsze zachowanie,
  // gdy SCYZORYK_GOOGLE_DRIVE_ROOT nie jest ustawione (ROOT==korzen).
  const MOUNT_ROOT = process.env.SCYZORYK_GOOGLE_DRIVE_ROOT || ROOT;
  ...
  async function checkAvailability(options = {}) {
    ...
    try {
      await withTimeout(fsp.readdir(ROOT), READDIR_TIMEOUT_MS, 'Katalog Dysku Google nie odpowiedzial w rozsadnym czasie (mount moze byc zawieszony).');
    } catch (err) {
  ...
  async function listDirectory(relativePath = '.') {
    ...
    try {
      const entries = await withTimeout(fsp.readdir(dir, { withFileTypes: true }), READDIR_TIMEOUT_MS, 'Folder na Dysku Google nie odpowiedzial w rozsadnym czasie (mount moze byc zawieszony).');
  ...
  // Sciezke lokalna (pod ROOT tego klienta) zamienia na argument dla rclone
  // CLI (nazwa remote'a + sciezka wzgledem korzenia CALEGO montowania -
  // patrz MOUNT_ROOT wyzej).
  function toRcloneArg(localAbsolutePath) {
    const resolvedMountRoot = fs.realpathSync(MOUNT_ROOT);
    const relFromRoot = path.relative(resolvedMountRoot, localAbsolutePath);
  ...
  // "fsp.copyFile" przez zamontowany katalog (--vfs-cache-mode full) pisze
  // TYLKO do lokalnego cache rclone i od razu wyglada na sukces - rzeczywista
  // synchronizacja z prawdziwym Dyskiem Google dzieje sie w tle, oddzielnie.
  // Jesli ta synchronizacja pozniej zawiedzie (siec, limit API, wygasly
  // token), plik jest widoczny TYLKO lokalnie (z cache tej maszyny) - appka
  // pokazuje sukces, ale kolega otwierajacy ten sam folder z innego
  // komputera (albo z poziomu samego Dysku Google w przegladarce) go nie
  // zobaczy. "rclone copyto" wywolane WPROST na remote (z pominieciem FUSE)
  // konczy sie sukcesem TYLKO gdy kopia faktycznie istnieje na Dysku -
  // uzywane tu zawsze, gdy RCLONE_REMOTE jest skonfigurowane. Bez tej
  // zmiennej (starszy/zapasowy tryb) zostaje dawne zachowanie przez FUSE.
  async function verifiedCopyViaRcloneRemote(sourceArg, destArg) {
    try {
      await execFileAsync('rclone', ['copyto', sourceArg, destArg], { timeout: 60000 });
    } catch (err) {
      throw new Error('Nie udalo sie skopiowac pliku na Dysku Google: ' + (err.stderr || err.message || String(err)));
    }
  }
  ...
  // bezposrednio na plikach w zamontowanym katalogu. Zostaje przez FUSE (nie
  // rclone-natywnie) - to READ z Dysku, VFS cache dla odczytu juz istniejacego
  // pliku jest wiarygodny (w przeciwienstwie do zapisu, patrz komentarz wyzej).
  ...
  async function copyFromLocal(localSourcePath, relativeDestPath) {
    ...
    if (RCLONE_REMOTE) {
      return withLimitedRetry(async () => {
        await verifiedCopyViaRcloneRemote(localSourcePath, toRcloneArg(dest));
        return dest;
      });
    }
  ...
  async function copyWithinDrive(relativeSourcePath, relativeDestPath) {
    ...
    if (RCLONE_REMOTE) {
      return withLimitedRetry(async () => {
        await verifiedCopyViaRcloneRemote(toRcloneArg(source), toRcloneArg(dest));
        return dest;
      });
    }
