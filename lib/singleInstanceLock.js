...
// Audyt rozdz. 28, P1: claim() tworzy pusty plik przez 'wx', a dopiero POTEM
// zapisuje tresc (tmp+rename) - miedzy tymi dwoma krokami inny proces moze
// zobaczyc istniejacy, ale jeszcze pusty plik. Bez ponowien to wygladaloby
// jak nieczytelny/pusty lock -> isOrphaned(null) uznaje go za osierocony ->
// drugi proces kasuje go i przejmuje w trakcie, gdy pierwszy wlasnie konczy
// zapis - realny wyscig dwoch startujacych rownoczesnie instancji. Ten sam
// wzorzec ponowien co juz poprawiona blokada drukowania (lib/printCoordinator.js#readLock).
const READ_RETRY_ATTEMPTS = 5;
const READ_RETRY_DELAY_MS = 40;

// Synchroniczny sleep (server.js woła acquireSingleInstanceLock() na starcie,
// zanim cokolwiek innego ruszy - musi zostac synchroniczne). Atomics.wait
// faktycznie usypia watek (nie zajeta petla), tak jak setTimeout/await w
// wersji async po stronie druku.
...
function readLockSync(file) {
  for (let attempt = 0; attempt < READ_RETRY_ATTEMPTS; attempt++) {
    try {
      ...
      sleepSyncMs(READ_RETRY_DELAY_MS);
    }
  }
...
function acquireSingleInstanceLock() {
  ...
  if (!isOrphaned(existing)) {
    return { acquired: false, existingPid: existing.pid ?? null, unreadable: Boolean(existing.unreadable) };
  }
  ...
  try {
    ...
    // Jesli po nieudanym claim() nikt locka nie trzyma, to NIE jest "druga
    // instancja" tylko awaria przejecia (EPERM/EBUSY/EACCES - antywirus,
    // OneDrive). Wolajacy musi to odroznic, bo inaczej konczy exit(0) i
    // launcher otwiera przegladarke na martwym porcie.
    return {
      acquired: false,
      existingPid: stillThere ? (stillThere.pid ?? null) : null,
      unreadable: Boolean(stillThere?.unreadable),
      claimError: stillThere ? null : String(err?.message || err)
    };
  }
}
