...
async function listPrinters() {
  let printersOutput = '';
  try {
    const { stdout } = await runCups('lpstat', ['-p']);
  ...
  const printers = [];
  const re = /^printer\s+(\S+)\s+is\s+([^.]+)\./gm;
  let m;
  while ((m = re.exec(printersOutput)) !== null) {
    printers.push({ name: m[1], status: m[2].trim(), isDefault: m[1] === defaultName });
  }
  return printers;
}
