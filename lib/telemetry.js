...
function createTelemetryService(options = {}) {
  ...
  const lockPath = path.join(telemetryDir, 'installation.lock');
  ...
  // Root i kazda z apek-dzieci maja WLASNA instancje telemetryService, ale
  // wszystkie wskazuja na ten sam statePath (wspolny dataRoot) - przy
  // pierwszym starcie kilka procesow rownoczesnie widzi "brak stanu" i bez
  // tej blokady kazdy rejestrowalby OSOBNA instalacje w monitorze (jeden
  // realny install -> kilka/kilkanascie "instalacji" w bocie, zglosil
  // wlasciciel). Blokada plikowa: 'wx' to atomowe "stworz-albo-rzuc" (dziala
  // tak samo na NTFS). Przegrani czekaja na plik stanu zapisany przez
  // zwyciezce zamiast rejestrowac wlasna tozsamosc.
    ...
    try {
      ...
      if (error && error.code === 'EEXIST') {
        try {
          ...
          if (Date.now() - stat.mtimeMs > LOCK_STALE_MS) fs.rmSync(lockPath, { force: true });
        } catch {}
        return false;
      }
      throw error;
  ...
  async function registerUnderLock() {
    ...
    try {
      return readState() || (await registerNewIdentity());
    } finally {
      releaseLock();
    }
  }
  ...
  async function ensureRegistered() {
    ...
    const own = await registerUnderLock();
    if (own) return own;

    // Nie zdobylismy blokady - ktos inny wlasnie rejestruje. Czekamy az
    // zapisze plik stanu zamiast rejestrowac wlasna tozsamosc rownolegle.
    const deadline = Date.now() + LOCK_WAIT_TIMEOUT_MS;
    while (Date.now() < deadline) {
      const state = readState();
      if (state) return state;
      await wait(LOCK_WAIT_STEP_MS);
    }

    // Posiadacz blokady najwyrazniej padl (crash) bez zapisania stanu i bez
    // zwolnienia blokady - stale-lock cleanup w acquireLock() usunie ja po
    // LOCK_STALE_MS, wiec kolejna proba w koncu przejmie rejestracje.
    return (await registerUnderLock()) || registerNewIdentity();
  }
